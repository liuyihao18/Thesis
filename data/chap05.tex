...
在该实验里，收发端采样率为1MHz，中心频率为915MHz，基带啁啾信号带宽为100KHz，长度为20ms，频移标签频率约253KHz\footnote{绘图时为了更清晰地展示效果，在不影响结果的前提下，对部分相位、幅度和频率数据进行了降采样。}。
...
图中双边带频率对称中心不是零点是由于实验中收发端不同步，存在载波频率偏移。
