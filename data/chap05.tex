...
在该实验中，标签放置于接收端30cm处以固定幅度2cm和频率0.66Hz进行往复运动。
...
在该实验中，发送端与接收端放置于相邻位置，标签距离收发端距离分别为0.5m、1m、1.5m、2m、2.5m和3m，并以固定幅度2cm和频率0.66Hz进行往复运动，从而测试不同标签距离对感知精度的影响。
图~\ref{fig:result-errorbar-outdoor-tag-distance}~展示了不同标签距离下三种方法的感知误差。
可以看到，与上一个实验相比，随着标签距离的增加，三种方法的感知精度下降都更为显著。
这主要是由于标签设计与发送端参数的选择，当啁啾信号带宽为500KHz且标签频率偏移在200KHz到400KHz之间时，频谱的部分重叠使得反向散射信号被强激励信号淹没，导致信噪比降低，从而影响感知精度。
...
值得注意的是，先前工作Palantir~\cite{jiang2021sense}通过相位转换的波形图的峰谷点来估计目标运动频率，这种方法对目标运动形式较为敏感，而本文提出的联合估计算法通过优化损失函数，能够更好地收集一段时间内的目标运动信息，从而提升了对不同运动形式的适应性。

\subsection{不同干扰级别对感知精度的影响}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.66\linewidth]{chap05/result/result_errorbar_outdoor_interference.pdf}
    \caption{不同干扰级别对感知精度的影响}
...
图~\ref{fig:result-errorbar-outdoor-interference}~展示了不同干扰级别下三种方法的感知误差。
可以看到，随着干扰级别的增加，三种方法的感知精度均有所下降，但联合估计算法得到的感知精度依然超过99.5\%。
这和~\ref{sec:anti-interference}~节中的抗干扰能力分析结果一致，只要干扰物体离收发端足够远，其影响便有限。
这表明了本文提出的方法在面对环境干扰时具有较强的鲁棒性，能够有效地提取目标运动特征。

\subsection{多标签场景}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.66\linewidth]{chap05/result/result_multitags_r_err_freq_combine.pdf}
    \caption{多标签场景下的感知精度}
...
为了模拟更多标签同时感知的场景，实验收集了7组发送端发送不同但正交啁啾信号时的数据，并将它们叠加在一起形成多标签场景的数据。
图~\ref{fig:result-errorbar-outdoor-multitags}~展示了多标签场景下三种方法的感知误差。
可以看到，由于啁啾信号相互正交且不同频移标签信号相互正交，本文提出的方法区分出了全部35个标签的感知信号，且平均频率感知准确率超过99.8\%。
不同标签感知准确率的波动可能源于标签反射系数的不同，导致不同标签信号的SNR不同，从而影响感知精度。
值得注意的是，实验中使用的5个标签中有2个标签频率间隔仅有1170Hz，虽然这个间隔很小，但是仍然满足了正交性约束$\left|f_2 - f_1\right|T \gg 1$，因此本文提出的方法依然很好地区分出了这两个标签信号。
...
更窄的啁啾信号和更长的符号长度能够有效缓解频谱重叠问题，并增加信号能量集中方法可以汇聚的信号能量，从而提升感知范围和精度。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\linewidth]{chap05/result/result_bar_indoor_room.pdf}
    \caption{室内实验结果}
...
而仅使用相位或幅度信息的方法在某些位置感知精度大幅下降，甚至无法感知目标。
...
本章通过实验，验证了远距离多目标运动特征感知系统原型的性能表现。
首先，通过模块测试，确认了本文提出各种方法的有效性和实时性，以及系统原型的可行性。
随后，通过室外实验，评估了系统在不同收发端距离、标签距离、目标运动形式和干扰级别下的感知精度，说明了本文提出的方法在各种条件下均表现出良好的感知性能。
最后，通过室内实验，展示了系统在复杂多径环境中的应用可行性。
总而言之，能量集中方法和窗函数确保了系统能够从接收信号中提取出微弱的标签反射信号，而收发端相位校正和标签频率漂移消除方法有效地修正了各种频率偏移对信号相位的影响，联合估计算法则通过结合相位和幅度信息，最终提升了系统的目标感知精度。
