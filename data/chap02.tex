...
此时，相当于将开关频率$f_{\text{tag}}$的方波信号调制到入射信号上：
\begin{equation}
...
在信号处理领域中，如果两个信号满足内积为零，那么这两个信号被称为正交信号。
...
考虑接收端收到两个不同频率单频信号的情形，设两个单频信号分别为：
\begin{equation}
...
若接收端想要从接收到的信号中提取出某一个单频信号的幅度和相位信息，可以通过将其与与该单频信号相同频率的单频信号进行内积运算来实现：
\begin{equation}
    \begin{aligned}
...
其中，$N$是采样点数，$k = \frac{f}{fs}N$是归一化的频率。
此时，内积运算定义为：
\begin{equation}
...
（2）\textbf{反向散射信号与激励信号彼此正交：}反向散射信号是激励信号经过频移调制后的信号，其频率远离了激励信号的频率，从而反向散射信号与激励信号彼此正交，可以被区分开来。这个性质使得反向散射信号可以在强直射信号的干扰下被有效提取出来，也使得利用激励信号进行无线同步成为可能。
...
如图~\ref{fig:vector2D}~所示，张等人的工作~\cite{zhang2021unlocking,zhang2020exploring}、谢等人的工作~\cite{xie2020combating}和江等人的工作~\cite{jiang2021sense}都采用了类似的表达方式，并且从中可以看出这些工作的局限性——由于接收端收到的信号是同频率信号的和$R$，因此难以区分出不同的信号源$H_1$和$H_2$，从而受限于单目标感知。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.72\linewidth]{chap02/vector3D.pdf}
    \caption{不同频率信号的相量叠加}
    \label{fig:vector3D}
\end{figure}

当接收端收到两个不同频率的信号时，可以将其认为是不同频率的相量平面的叠加，如图~\ref{fig:vector3D}~所示。
这也对应了上一小节中提到的不同频率信号的正交性原理，因为不同频率的信号代表了不同的相量平面，所以不同频率的信号可以被区分开来。
本文提出的系统正是利用了这一原理，从而使得接收端可以区分出不同感知目标的信号，进而实现多目标同时感知的功能。

\section{信号传播建模}

本节将介绍如何通过无线信号的特征（如幅度、相位等）变化来推断环境中目标的运动状态变化。
这是本工作的理论基础，当装有频移反向散射标签的目标运动状态发生变化时，经由其反射的信号的特征也会发生变化，因此可以通过分析信号特征的变化来得出目标的运动状态变化。
...
其中，$\varphi_{\text{RX}}$是接收端载波的初相位，$\alpha(\tau)$是信号的幅度，随着传播距离的增加而减小。
接收端在接收到信号后，进行时间窗口对齐，得到：
\begin{equation}
    \begin{aligned}
...
如图~\ref{fig:propagation1}~所示，在存在有频移反向散射标签的场景中，接收端收到的信号除了包含激励信号外，还包含有来自标签的反射信号。
...
在多目标感知的场景中，不同的感知目标会装备不同频移频率的反向散射标签，并且满足$\left|f_{\text{tag}_i} - f_{\text{tag}_j} \gg \frac{1}{T}\right|$正交性条件，于是，不同目标的反向散射信号也可以被区分开来：
\begin{equation}
...
至此，本文完成了信号传播模型的建立，通过激励信号和反射信号的正交性，成功提取出多个目标的反射信号特征。
值得注意的是，虽然本文在建模过程中假设了基带信号是啁啾信号，但实际上任何基带信号类型的感知都可以使用本文的模型进行描述，只要该信号可以解调成单频信号。
...
另一类是动态信号$r_{D_i}(t)$，包括被动态感知目标反射然后被接收端接收的反射信号。
在章节~\ref{sec:vector}~中，已经介绍了如何将信号表示为复数平面上的相量。
因此，也可以把静态信号看作复数平面上不变的静态相量，而把动态信号看作复数平面上不断伸缩与转动的动态相量。

传统的感知模型不使用频移反向散射标签区分信号，因此静态信号和动态信号都包含在激励信号当中，此时接收机收到的信号可以表示为：
\begin{equation}
    r(t) = \sum_{i} r_{S_i}(t) + \sum_{i} r_{D_i}(t) = r_E(t)
\end{equation}

此时，动态信号和静态信号混杂在一起，难以区分开来。
而多目标感知在这种情况下将变得更加困难，因为不同目标的动态信号也混杂在一起。
...
而本文引入了频移反向散射标签，因此静态信号和动态信号被分别划分到了激励信号和反向散射信号当中：
\begin{equation}
    \begin{aligned}
...
基于该正交感知模型，本文提出的系统无需引入额外的设备支持或先验条件，结合复杂度较低的算法，就可以实现多目标同时感知。

诚然，动态信号也存在多径的叠加，但是每一个反向散射信号仅包含一个感知目标的信息，也就是说有：
\begin{equation}
    r_{B_i}(t) = \sum_{j} r_{D_{i_j}}(t)
\end{equation}

这个问题常常在过去的工作中被忽略掉，但是本文指出，这个问题虽然会影响目标运动幅度的感知，但并不会影响目标运动频率的感知。
后续的实验结果也说明了这一点。

\section{本章小结}

本章首先介绍了啁啾信号和反向散射信号，给出了它们的数学表达式和基本特性。
接着，本章重点介绍了信号正交的基本原理，说明了不同频率的单频信号彼此正交，可以被区分开来，由此本文提出了核心的理论基础。
随后，本章建立了信号传播模型，描述了如何通过分析信号特征的变化来推导目标的运动状态变化。
最后，本章进一步完善了信号传播模型，建立了面向频移反向散射标签的感知模型，并说明本文如何通过频移反向散射标签区分静态信号和动态信号，从而实现多目标同时感知。
为了将理论模型应用在实际系统中，还需要解决一系列挑战，在下一章节中将展开介绍这些挑战，并给出相应的解决方案。
