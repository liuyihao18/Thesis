...
虽然第一项由于$f_\text{tag}$通常只有几十kHz量级，而$\tau - \tau_2$只有纳秒级，使得其可忽略不计，但是第二项$\psi_{\text{tag}}$\footnote{$\Delta \psi_{\text{tag}} \approx 2 \pi \Delta f_\text{tag} \Delta t$可以作为一个量级意义上的估计。}会引入显著的相位偏移，影响感知精度。
...
\subsubsection{傅里叶变换引入的固定偏移}

当标签的频率发生严重漂移时，反向散射信号在傅里叶变换后的频点也可能出现移动，这会导致固定的相位偏移。
假设时刻$t_1$的信号频率为$f_1$，时刻$t_2$的信号频率为$f_2$，且两者之间的时间间隔为$\Delta t = t_2 - t_1$，则该相位偏移为$\pi(f_2 - f_1)\Delta t$。
